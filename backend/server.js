...
function handleArchive(req, res, format) {
    ...
    bb.on('file', (filenames, file, originalFilename) => {
...
app.post('/upload-image', (req, res) => {
    try {        
        ...
        bb.on('file', async (filenames, file, filename, encoding, mimetype) => {
            //first check if the file exists in S3
            ...
            if (existsInS3) {
                ...
                if (cachedFile) {
                    ...
                    //if not in Redis and it's larger than the threshold, upload to S3
                    const resizeTransform = sharp().resize(100);
                        ...
                        if (existsInS3) {
                          ...
                          if (cachedFile) {
                            ...
                            resolve({ name: originalFilename.filename, file: Buffer.from(cachedFile, 'base64') });
                              ...
                              const processedImageBuffer = await sharp(fileBuffer).resize(100).toBuffer();
                              //if not in Redis and it's larger than the threshold, upload to S3
                            if (processedImageBuffer.length > SIZE_THRESHOLD) {
